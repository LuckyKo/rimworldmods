...
2. Misbehavior Types Implementation
  ...
  Level 3 - Property Damage (High Severity):
   ...
   - Breaking workbenches or other furniture (not damage but putting them in the broken state that needs repairing with components)
  ...
  Level 4 - Dangerous Behavior (Critical Severity):
   ...
   - Leak location of colony to raiders when using radio. trigger a raid event one day later. - done
   - Run away from home (become global pawn. similar to giveUp from vanilla but a job instead of mental state so the player can interrupt it easily)
