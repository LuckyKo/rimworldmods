...
namespace SocialInteractions
{
    ...
    public class QwenApiClient : IDisposable
    {
        ...
        public QwenApiClient(string apiUrl, string modelName, string apiKey)
        {
            _apiUrl = apiUrl;
            ...
            if (!string.IsNullOrEmpty(_apiKey))
            {
                try
                {
                    ...
                    {
                        // DashScope Qwen API uses Authorization header with Bearer token
        ...
        public async Task<string> GenerateText(string prompt, int? maxLength = null, float? temperature = null, List<string> stopSequence = null, bool? enableXtcSampling = null, int? topK = null, float? topP = null, float? minP = null)
        {
            ...
            try
            {
                ...
                // Convert to JSON
                DataContractJsonSerializer serializer = new DataContractJsonSerializer(typeof(QwenApiRequest));
                ...
                SLog.Message(string.Format("[SocialInteractions] Qwen API Request: {0}", jsonContent));
                ...
                DataContractJsonSerializer deserializer = new DataContractJsonSerializer(typeof(QwenApiResponse));
                MemoryStream responseStream = new MemoryStream(Encoding.UTF8.GetBytes(responseBody));
                QwenApiResponse apiResponse = (QwenApiResponse)deserializer.ReadObject(responseStream);
            ...
            {
                SLog.Warning(string.Format("[SocialInteractions] QwenApiClient: HTTP request failed: {0}", ex.Message));
                return null;
            }
            catch (Exception ex)
            {
