    ...
    public class JobDriver_GoOnDate : JobDriver
    {
        ...
        protected override System.Collections.Generic.IEnumerable<Toil> MakeNewToils()
        {
            ...
            assignJoyJob.initAction = () =>
            {
                ...
                System.Collections.Generic.List<System.Tuple<Thing, JoyGiverDef>> joySpots = DatingManager.FindJoySpotFor(initiator, partner);

                if (joySpots != null && joySpots.Any())
                {
                    // For simplicity, pick the first available joy spot
                    System.Tuple<Thing, JoyGiverDef> chosenSpot = joySpots.First();
                    ...
                    if (joyGiver != null && joyGiver.jobDef != null && initiator.jobs != null && partner.jobs != null)
                    {
                        ...
                        if (joyThing == null || !joyThing.Spawned || !joyThing.Position.IsValid || joyThing.Map == null)
                        {
                            Log.Message(string.Format("[SocialInteractions] JobDriver_GoOnDate: Invalid joyThing found. JoyThing: {0}, Spawned: {1}, Position Valid: {2}, Map: {3}. Ending date.", joyThing != null ? joyThing.LabelShort : "NULL", joyThing != null ? joyThing.Spawned.ToString() : "NULL", joyThing != null ? joyThing.Position.IsValid.ToString() : "NULL", joyThing != null && joyThing.Map != null ? joyThing.Map.ToString() : "NULL"));
                        ...
                        Log.Message(string.Format("[SocialInteractions] JoyThing Details: X={0}, Y={1}, Z={2}, MapSizeX={3}, MapSizeZ={4}, InBounds={5}", joyThing.Position.x, joyThing.Position.y, joyThing.Position.z, joyThing.Map.Size.x, joyThing.Map.Size.z, joyThing.Position.InBounds(joyThing.Map)));
                        // Assign joy job to initiator
                        Job initiatorJoyJob = JobMaker.MakeJob(joyGiver.jobDef, joyThing);
