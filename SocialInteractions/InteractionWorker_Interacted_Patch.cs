                ...
                if (interactionDef != null)
                {
                        ...
                        (interactionDef == InteractionDefOf.DisturbingChat && SocialInteractions.Settings.enableDisturbingChat))
                    {
                        ...
                        if (isLlmEnabled)
                        {
                            ...
                            {
                                SLog.Message(string.Format("[SocialInteractions] Interaction {0} has a valid prompt, creating jobs.", interactionDef.defName));
                                ...
                                SLog.Message("[SocialInteractions] Jobs created successfully.");
                            }
                            else
                            {
                                SLog.Message(string.Format("[SocialInteractions] Interaction {0} does not have a valid prompt, showing default bubble without creating jobs.", interactionDef.defName));
                        ...
                        {
                            SLog.Message(string.Format("[SocialInteractions] Interaction {0} is not LLM-enabled, showing default bubble without creating jobs.", interactionDef.defName));
                            // For non-LLM interactions, show the default text
                            SpeechBubbleManager.ShowDefaultBubble(initiator, subject);
                        }
                    }
                    ...
                    {
                        SLog.Message(string.Format("[SocialInteractions] Interaction {0} does not match criteria.", interactionDef.defName));
                    }
                }
                else
                {
                    SLog.Warning("[SocialInteractions] InteractionDef is null, skipping job creation.");
                }
            }
            else
            {
                SLog.Message("[SocialInteractions] pawnsStopOnInteraction is false, using default behavior.");
            }
        }
    }
}